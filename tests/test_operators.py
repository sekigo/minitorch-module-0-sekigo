...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    # Проверка возрастания: sigmoid(a+1) > sigmoid(a)
    assert sigmoid(a + 1e-1) >= s
...
def test_transitive(a: float, b: float, c: float) -> None:
    ...
    if lt(a, b) and lt(b, c):
        ...
        # Если не выполняется посылка, ничего не проверяем
        assert True
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    left = add(minitorch.operators.sum(ls1), minitorch.operators.sum(ls2))
